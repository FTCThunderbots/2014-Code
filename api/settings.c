...
const int ENCODER_TICKS_PER_ROTATE_DEGREE = 0; // for rotating in place. just guess and check
...
int delete_this_function_when_encoder_values_are_referenced_somewhere_else() {
	...
	return ENCODER_TICKS_PER_ROTATION + ENCODER_TICKS_PER_DRIVE_INCH
		+ ENCODER_TICKS_PER_STRAFE_INCH + ENCODER_TICKS_PER_ROTATE_DEGREE + ENCODER_TICKS_PER_SWING_DEGREE;
}
